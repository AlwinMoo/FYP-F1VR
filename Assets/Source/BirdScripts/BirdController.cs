...
public class BirdController : MonoBehaviour {
	...
	void Start () {
		...
		if(robin){
			myBirdTypes.Add ("robin");
		}
		if (blueJay){
			myBirdTypes.Add ("blueJay");
		}
		if(cardinal){
			myBirdTypes.Add ("cardinal");
		}
		if(chickadee){
			myBirdTypes.Add ("chickadee");
		}
		if(sparrow){
			myBirdTypes.Add ("sparrow");
		}
		if(goldFinch){
			myBirdTypes.Add ("goldFinch");
		}
		if(crow){
			myBirdTypes.Add ("crow");
		}
	...
	IEnumerator UpdateTargets(){
		...
		while(true){
			...
			foreach(Collider hit in hits){
				if (hit.tag == "groundTarget" && !birdGroundTargets.Contains (hit.gameObject)){
					birdGroundTargets.Add (hit.gameObject);
				}
				if (hit.tag == "perchTarget" && !birdPerchTargets.Contains (hit.gameObject)){
